          ...
          <div className="flex flex-col gap-6 md:gap-8 lg:gap-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 md:gap-6 lg:gap-4">
              ...
              <div className="flex flex-col gap-3 md:gap-4">
                ...
                <div className="flex flex-col gap-2 md:gap-3">
                  <ContactItem icon={<PhoneIcon />} text="+971 26 677 448" href="tel:+97126677448" />
                  <ContactItem icon={<EmailIcon />} text="smile@skydc.ae" href="mailto:smile@skydc.ae" />
...
function ContactItem({ icon, text, href }: { icon: React.ReactNode; text: string; href?: string }) {
  const content = (
    <>
      ...
      <span className="text-black text-[13px] md:text-[14px] leading-[1.5] break-words">{text}</span>
    </>
  )
  return (
    <div className="flex items-start gap-3">
      {href ? (
        <a href={href} className="flex items-start gap-3 hover:underline no-underline text-inherit">
          {content}
        </a>
      ) : (
        content
...
function SocialIcon({ href, children }: { href: string; children: React.ReactNode }) {
  ...
  const className = "w-[32px] h-[32px] md:w-[36px] md:h-[36px] flex items-center justify-center transition-opacity hover:opacity-70"
  if (isInternal) {
    return (
